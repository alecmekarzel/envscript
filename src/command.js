...
program.version(package.version).description(package.description).usage('<script>')

program.arguments('<script>').action(function (scriptName) {
	...
	if (!userPackage.envscripts[scriptName])
		return console.error('ERROR: No envscript with name "' + script + '" was found!')
